 ...
 * over memory bandwidth (b/w) for the software. A user controls the resource
 * by indicating the percentage of maximum memory b/w.
 *
 ...
 * cache and memory b/w resources constraints.
 *
 ...
 * Each resource (L3 cache, memory b/w) has its own line and format.
 *
 * L3 cache schema:
 ...
 *
 * Memory bandwidth (b/w) schema:
 * It has allocation values for memory b/w on each socket, which contains L3
 * cache id and memory b/w percentage.
 * 	Format: "MB:<cache_id0>=bandwidth0;<cache_id1>=bandwidth1;..."
 * For example, on a two-socket machine, the schema line could be "MB:0=20;1=70"
 *
 * The minimum b/w percentage value for each CPU model is predefined and can
 * be looked up through "info/MB/min_bandwidth". The b/w granularity that is
 * allocated is also dependent on the CPU model and can be looked up at
 * "info/MB/bandwidth_gran". The available b/w control steps are:
 ...
 * 0x7ff and the max CBM length is 11 bits, and minimum memory b/w of 10% with
 * a memory b/w granularity of 10%.
 *
 ...
 * maximum memory b/w of 20% on socket 0 and 70% on socket 1.
 *
 * "linux": {
 *     "intelRdt": {
...
// Get the read-only memory b/w information
...
func (m *IntelRdtManager) GetStats() (*Stats, error) {
	...
	// The read-only L3 cache and memory b/w schemata in root
	...
	// The L3 cache and memory b/w schemata in 'container_id' group
	...
	if IsMbaEnabled() {
		// The read-only memory b/w information
		...
		// The memory b/w schema in 'container_id' group
		for _, schema := range schemaStrings {
...
func (m *IntelRdtManager) Set(container *configs.Config) error {
	...
	//
	//
	// About memory bandwidth (b/w) schema:
	// It has allocation values for memory b/w on each socket, which
	// contains L3 cache id and memory b/w percentage.
	// 	Format: "MB:<cache_id0>=bandwidth0;<cache_id1>=bandwidth1;..."
	// For example, on a two-socket machine, the schema line could be:
	// 	"MB:0=20;1=70"
	//
	// The minimum b/w percentage value for each CPU model is predefined
	// and can be looked up through "info/MB/min_bandwidth". The b/w
	// granularity that is allocated is also dependent on the CPU model
	// and can be looked up at "info/MB/bandwidth_gran". The available b/w
	...
	if container.IntelRdt != nil {
		...
		memBwSchema := "MB:"
		for _, m := range container.IntelRdt.MemBwSchema {
			memBwSchema = memBwSchema + m.MemBwString()
		}
		if memBwSchema != "" {
